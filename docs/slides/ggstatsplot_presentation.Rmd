...
`r emo::ji("check")` accept `symbols` as arguments (`x = var`)<br>
...
In addition to `output = "plot"`, this function can also be used to get a <font color="blue">dataframe</font> of results:

.font50[
```{r ggcorrmat_2, tidy=FALSE}
...
`fitdistr`, `fixest`, `flexsurvreg`, `gam`, `Gam`, `gamlss`, `garch`, `geeglm`,
`glmc`, `glmerMod`, `glmmTMB`, `gls`, `glht`, `glm`, `glmm`, `glmmadmb`,
`glmmPQL`, `glmRob`, `glmrob`, `glmx`, `gmm`, `HLfit`, `hurdle`, `ivFixed`,
...
`margins`, `mcmc`, `mcmc.list`, `MCMCglmm`, `mclogit`, `mice`, `mmclogit`,
`mediate`, `metafor`, `merMod`, `merModList`, `metaplus`, `mixor`, `mjoint`,
`mle2`, `mlm`, `multinom`, `negbin`, `negbinmfx`, `negbinirr`, `nlmerMod`,
`nlrq`, `nlreg`, `nls`, `orcutt`, `orm`, `plm`, `poissonmfx`, `poissonirr`,
`polr`, `ridgelm`, `riskRegression`, `rjags`, `rlm`, `rlmerMod`, `robmixglm`,
`rq`, `rqss`, `rrvglm`, `scam`, `semLm`, `semLme`, `slm`, `speedglm`, `speedlm`,
...
# Best practices in statistical tests defaults

--

The default tests follow the best practices. 
...
# Toggling between different statistical approaches
...
- Minimal amount of code needed for all functions (typically only `data`, `x`,
  and `y`). This minimizes chances of error and makes for tidy scripts.
...
- Requires non-trivial level of statistical proficiency (but only plots
  without statistics can also be used).
