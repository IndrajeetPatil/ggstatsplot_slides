...
<br>
📊 **information-rich** plots with **statistical details** <br>
 
📝 plots suitable for scholarly publications and **faster** (exploratory) data analysis
]
...
The **grammar of graphics** is a powerful framework [(Wilkinson, 2011)](https://www.google.com/books/edition/_/iI1kcgAACAAJ?hl=en&sa=X&ved=2ahUKEwiGl8rJ2KztAhWyElkFHa8NAvkQre8FMBR6BAgMEAc) and can help you make
...
[`statsExpressions`](https://indrajeetpatil.github.io/statsExpressions/), statistical processing backend for `ggstatsplot`, can be used to get
**only the dataframes**.
...
`r emo::ji("x")` Limited types of **plots** available. 

--

`r emo::ji("x")` Limited number of statistical **tests** available (no support
for multivariate statistics, time series, etc.). This will **always** be the
case. 🤷

--

`r emo::ji("x")` Expects a non-trivial level of statistical proficiency (but
plots without statistics can still be useful).

--

`r emo::ji("x")` **Faceting** does not work (since there are no corresponding
`geom_` s).

--

`r emo::ji("x")` For the same reason, plots are not `gganimate`-friendly.
...
- Star the GitHub repo (increases visibility) ⭐
