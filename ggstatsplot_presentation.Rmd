...
.right-column[.font150[
In short, `ggstatsplot` provides a collection of <font color="blue">*information-rich*</font> plots with <font color="blue">*statistical details*</font> suitable either for scholarly publications or for quick (exploratory) statistical analysis.
]
]
...
Most of the regression models covered by `broom` and `broom.mixed` packages will
be covered here-

--

`aareg`, `anova`, `aov`, `aovlist`, `Arima`, `biglm`, `btergm`, `cch`, `clm`,
`clmm`, `confusionMatrix`, `coxph`, `ergm`, `felm`, `fitdistr`, `glmerMod`,
`glmmTMB`, `gls`, `Gam`, `gamlss`, `garch`, `glm`, `glmmadmb`, `glmmTMB`,
`glmrob`, `gmm`, `ivreg`, `lm`, `lm.beta`, `lmerMod`, `lmodel2`, `lmrob`,
`MCMCglmm`, `mediate`, `mjoint`, `mle2`, `multinom`, `nlmerMod`, `nlrq`, `nls`,
`orcutt`, `plm`, `polr`, `ridgelm`, `rlm`, `rlmerMod`, `rq`, `speedlm`,
...
.left-code[
```{r ggcoefstats_2, eval = FALSE, message = FALSE, warning = FALSE}
...
.right-plot[
```{r ggcoefstats_2, ref.label = 'ggcoefstats_2', echo = FALSE, message = FALSE, warning = FALSE}
```
]
...
.right-plot[
```{r ggcoefstats_3, ref.label = 'ggcoefstats_3', echo = FALSE}
```
]
...
You can also use ggstatsplot just to extract the statistical details for your
custom plot. For example-
...
corresponding subtitle making
[helper functions](https://indrajeetpatil.github.io/ggstatsplot/reference/index.html#section-helper-functions-for-preparing-statistics-subtitles).
]
]
...
- More breathing room for theory and other text.

--

- No more mixing of stats and textual results.
